...
class Supervise:
    ...
    def create_model(self, state_dim, img_dim):
        ...
        img_layer2 = tf.layers.max_pooling2d(inputs=img_layer1, pool_size=[2, 2], strides=2)
        img_layer3 = tf.layers.conv2d(inputs=img_layer2, filters=32, kernel_size=[5, 5], padding="VALID", activation=tf.nn.relu)
        img_layer4 = tf.layers.max_pooling2d(inputs=img_layer3, pool_size=[2, 2], strides=2)
        img_layer5 = tf.layers.conv2d(inputs=img_layer4, filters=64, kernel_size=[4, 4], padding="VALID", activation=tf.nn.relu)
        img_layer6 = tf.layers.max_pooling2d(inputs=img_layer5, pool_size=[2, 2], strides=2)
    ...
    def create_method(self):
        ...
        self.cost = tf.reduce_mean(tf.square(self.steer - self.steer_ground_truth))
